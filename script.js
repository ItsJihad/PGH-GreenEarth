...
ButtonCreator=(buttonData)=>{
  ...
  buttonData.forEach((button)=>{
    ...
    categorybutton.classList = "categorybuttons  m-2 sm:m-0 bg-[#F0FDF9] px-2.5 py-2 text-black rounded hover:bg-green-200 ";
  ...
  cardData.forEach((card) => {
      ...
      Divplacer.innerHTML = `
            ...
            <div class="p-4 bg-white rounded-lg flex flex-col gap-3 " id="${card.id}">
                ...
                <div class="flex-1 flex flex-col gap-2">
                    <div class="text-gray-800 text-sm font-semibold" onclick="my_modal_1.showModal()" >${card.name}</div>
                    ...
                    <div class="flex-1 text-right text-gray-800 text-sm font-semibold">৳ ${card.price}</div>
                    </div>
                </div>
                </div>
                <button class=" Addtocart w-full px-5 py-3 bg-green-700 hover:bg-yellow-300 rounded-full text-white text-base font-medium">
